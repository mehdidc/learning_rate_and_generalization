...
def augment(X, angle_range=(-15, 15), scale_range=(0.85, 1.15), alpha_range=(36, 38), sigma_range=(5, 6), n_jobs=1):
    X = X[:, 0]
    X = rotate_scale(X, min_angle=angle_range[0], max_angle=angle_range[1], min_scale=scale_range[0], max_scale=scale_range[1], n_jobs=n_jobs)
    X = elastic_transform(X, min_alpha=alpha_range[0], max_alpha=alpha_range[1], min_sigma=sigma_range[0], max_sigma=sigma_range[1], n_jobs=n_jobs)
    X = X[:, None, :, :]
